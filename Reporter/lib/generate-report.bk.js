...
const output_file = './.jsonreport/output.json';
...
const DEFAULT_REPORT_NAME = 'Cucumber HTML Reporter';
var updater = require('jsonfile-updater')
const failedStepsMap = new Map();
        

// // Utility function to calculate percentage
// function _calculatePercentage(part, total) {
//   return total === 0 ? '0.00' : ((part / total) * 100).toFixed(2);
// }

// Utility function to update JSON data
...
function generateReport(options) {
  fs.ensureDir('./.jsonreport')
  
  fs.writeJson('./.jsonreport/output.json', {
    "ApplicationName":"",
    "ExecutionType":"",
    "Environment":"",
    "Features_Total": 0,
    ...
    "Features_Failed": 0,
    "Features_Skipped":0,
    "Features_PassedPercentage":"00.00",
    "Features_FailedPercentage": "00.00",
    "Features": [
    ],  
    "Scenarios_Total": 0,
    "Scenarios_Passed": 0,
    "Scenarios_Failed": 0,
  "Scenarios_Skipped": 0,
  "Scenarios_PassedPercentage": "00.00",
  "Scenarios_FailedPercentage": "00.00",
  "ExecutionStartTime":"",
  "ExecutionEndTime":"",
  "TotalExecutionTime":"",
  }, err => {
  ...
  const pageTitle = options.pageTitle || 'Cucumber HTML Reporter';
...
// // Iterate through features and add up their durations
// suite.features.forEach((feature) => {
//   totalDuration += feature.duration;
// });

// // Format the total duration using the formatDuration function
// const formattedTotalDuration = formatDuration(totalDuration);

// // console.log(formattedTotalDuration)
// // //const formattedTotalDuration = `${padZero(hours)}:${padZero(minutes)}:${padZero(seconds)}.${padZero(Math.floor(totalDurationInSeconds * 1000) % 1000)}`;
// // console.log(formattedTotalDuration)


// // Add a worksheet for execution summary
// const executionSummaryWorksheet = workbook.addWorksheet('Execution Summary');

// // Set up the header row for execution summary
// executionSummaryWorksheet.addRow([
//   'Total Features',
//   'Feature Pass Count',
//   'Feature Fail Count',
//   'Total Scenarios',
...
// // Calculate the overall execution status
// const totalFeatures = suite.features.length;
// const totalScenarios = suite.scenarios.total;
// const passedScenarios = suite.scenarios.passed;
// const failedScenarios = suite.scenarios.failed;
// const skippedScenarios = suite.scenarios.skipped;
// const pendingScenarios = suite.scenarios.pending;
// const ambiguousScenarios = suite.scenarios.ambiguous;
// const notDefinedScenarios = suite.scenarios.notDefined;
// //const totalDuration = suite.totalTime;

// // Add the execution summary row
// executionSummaryWorksheet.addRow([
//   totalFeatures,
//   suite.featureCount.passed,
//   suite.featureCount.failed,
...
// ]);
// // // Add custom data rows
// // if (suite.customData && suite.customData.data) {
...
// //   });
// // }
// // // Add a blank row to separate the execution summary from the custom data
// // executionSummaryWorksheet.addRow([]);

// // // Add a header row for the custom data with bold font
// // executionSummaryWorksheet.addRow([
// //   { value: 'Custom Data', font: { bold: true } },
// //   { value: 'Value', font: { bold: true } },
// // ]);
...
// // Add custom data rows
// if (suite.customData && suite.customData.data) {
//   suite.customData.data.forEach((row, index) => {
//     if (index === 0) {
//       executionSummaryWorksheet.addRow([
//         { value: row.label, font: { bold: true } },
//         { value: row.value, font: { bold: true } },
//       ]);
//     } else {
//       executionSummaryWorksheet.addRow([
//         row.label,
...
//   });
// } 
// // Add a worksheet for features
// const featuresWorksheet = workbook.addWorksheet('Features');

// // Set up the header row for features
// featuresWorksheet.addRow([
//   'Feature',
//   'Scenarios',
//   'Passed',
//   'Failed',
...
// suite.features.forEach((feature) => {
//   const totalScenarios = feature.scenarios.total;
//   const passedScenarios = feature.scenarios.passed;
//   const failedScenarios = feature.scenarios.failed;
//   const skippedScenarios = feature.scenarios.skipped;
//   const pendingScenarios = feature.scenarios.pending;
//   const ambiguousScenarios = feature.scenarios.ambiguous;
//   const notDefinedScenarios = feature.scenarios.notDefined;

//   const passedPercentage = (passedScenarios / totalScenarios) * 100;
//   const failedPercentage = (failedScenarios / totalScenarios) * 100;
//   const skippedPercentage = (skippedScenarios / totalScenarios) * 100;
//   const pendingPercentage = (pendingScenarios / totalScenarios) * 100;
//   const ambiguousPercentage = (ambiguousScenarios / totalScenarios) * 100;
//   const notDefinedPercentage = (notDefinedScenarios / totalScenarios) * 100;
...
//     ambiguousPercentage.toFixed(2),
//     notDefinedPercentage.toFixed(2),
//   ]);
// });


// // Save the Excel file
// const excelFile = path.resolve(reportPath, 'report.xlsx');
...
//   executionSummary: {
//     totalFeatures,
//     passedFeatures: suite.featureCount.passed,
//     failedFeatures: suite.featureCount.failed,
//     totalScenarios,
//     passedScenarios,
//     failedScenarios,
//     skippedScenarios,
...
//       passedPercentage: (passedScenarios / totalScenarios) * 100,
//       failedPercentage: (failedScenarios / totalScenarios) * 100,
//       skippedPercentage: (skippedScenarios / totalScenarios) * 100,
//       pendingPercentage: (pendingScenarios / totalScenarios) * 100,
//       ambiguousPercentage: (ambiguousScenarios / totalScenarios) * 100,
//       notDefinedPercentage: (notDefinedScenarios / totalScenarios) * 100,
//     };
//   }),
// };
...
// // Save the JSON file
// const jsonFile = path.resolve(reportPath, 'report.json');
...
// Add a blank row to separate execution summary from custom data
executionSummaryWorksheet.addRow([]);

// Add a header row for custom data with bold font
executionSummaryWorksheet.addRow([
  { value: 'Custom Data', font: { bold: true } },
  { value: 'Value', font: { bold: true } },
]);

// Add custom data rows (Excel)
if (suite.customData && suite.customData.data) {
  suite.customData.data.forEach((row) => {
    executionSummaryWorksheet.addRow([
      row.label,
...
// Add a blank row to separate the custom data from other sections (if needed)
executionSummaryWorksheet.addRow([]);
...
const reportData = {
  ...
  customData: suite.customData ? suite.customData.data : [],
};
...
function formatDuration(duration) {
  ...
  if (!disableLog) {
    console.log(
      ...
      `\n
=====================================================================================
  Cucumber HTML report generated in:
...
=====================================================================================\n`
  ...
  async function _parseFeatures(suite) {
    suite.features.forEach((feature) => {
      feature.scenarios = {
        passed: 0,
        failed: 0,
      ...
      var Passed_ScenarioName = []
      var Failed_ScenarioName = []
      var Skipped_ScenarioName = []
      var failedscenarioarr = []
     
      feature.elements.forEach((element,index) => {
      // console.log(index)
        if(element.failed>0){
        //  console.log(failedsteparr[index])
          failedscenarioarr = {
            Failed_ScenarioName: element.name,
            Failed_Step:failedStepsMap.get(element.name, failedsteparr)
          }
          // console.log("failedsteparr[index]:", failedsteparr[index])
          // console.log("failedscenarioarr:", failedscenarioarr)
          Failed_ScenarioName.push(failedscenarioarr)
          // console.log(Failed_ScenarioName)   
        }
      
        else if(element.passed>0){
         // console.log(element.passed)
          Passed_ScenarioName.push(element.name)
        } 
        
        if(element.skipped>0){
          Skipped_ScenarioName.push(element.name)
        }
       // console.log(element.name);
    });
      var featurearr = {
        "FeatureName": feature.name,
        "Scenarios_Count": feature.scenarios.total,
        "Scenarios_Passed_Count":feature.scenarios.passed,
        "Scenarios_Failed_Count":feature.scenarios.failed,
        "Scenarios_Skipped_Count":feature.scenarios.skipped,
        "Scenarios_Passed": Passed_ScenarioName,
        "Scenarios_Failed": Failed_ScenarioName,
        "Scenarios_Skipped": Skipped_ScenarioName,
        
      }
     // console.log(feature.name)
      featurename.push(featurearr)
    ...
    await updater(output_file).set(	"Features_Total",	suite.featureCount.total)
    await updater(output_file).set(	"Features_Passed",	suite.featureCount.passed)
    await updater(output_file).set(	"Features_Failed",	suite.featureCount.failed)
    await updater(output_file).set(	"Features_Skipped",	suite.featureCount.skipped)
    await updater(output_file).set(	"Features_PassedPercentage",	_calculatePercentage(suite.featureCount.passed,suite.featureCount.total))
    await updater(output_file).set("Features_FailedPercentage",	_calculatePercentage(suite.featureCount.failed,suite.featureCount.total))
    
    await updater(output_file).set(	"Features",	featurename)   
 await updater(output_file).set(	"Scenarios_Passed",	suite.scenarios.passed)
 await updater(output_file).set(	"Scenarios_Failed",	suite.scenarios.failed)
 await updater(output_file).set(	"Scenarios_Skipped", suite.scenarios.skipped)

 await updater(output_file).set(	"Scenarios_Total", suite.scenarios.total)
await updater(output_file).set(	"Scenarios_PassedPercentage",	suite.scenarios.passedPercentage)
await updater(output_file).set(	"Scenarios_FailedPercentage",	suite.scenarios.failedPercentage)
}
  ...
  function _parseSteps(scenario) {
   ...
   //   console.log(scenario.name)
      if(step.result.status == 'failed'){
        var rca = step.result.error_message.split("(")[0].split('=').join('').split('\n').join('')
      //  console.log(step.keyword+ " " +step.name+" " +rca)

      failedsteparr = 
      {
      Failed_StepName : step.keyword+ " " +step.name,
      Failed_Reason:rca
      }
      failedStepsMap.set(scenario.name, failedsteparr);


    //console.log(failedStepsMap)
    }
